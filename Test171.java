...
○ 객체 직렬화(Object Serialization)는
   ...
   (객체의 속성(변수)만을 보낸다. 중요한건 값을 보내야한다.)
...
○ 객체 직렬화의 장점
   ...
   이 객체 직렬화를 사용하여 편하게 객체 상태를 저장할 수 있다.
   ...
   ※ RMI(Remote Method Invocation)
      ...
      // 내가 보낸 객체의 변수를 저쪽에서 받고 함수로 조작가능하게 하는거

   ※ Bean(빈)
      CS(Client 와 Server) 구조적 모델에서 ^서버측 구조^에 해당하며
      ^재사용 가능^한 소프트웨어 개체를 만들 수 있게 하는 ^컴포넌트 기술^
...
○ Serializable 인터페이스
   ...
   ※ 멤버 변수가 static 으로 선언된 겨우(즉, 클래스 변수일 경우)
      객체 직렬화의 대상에서 제외된다. (인스턴스 변수들만 직렬화의 대상)
    
○ Object Stream (직렬화 기능을 포함하고 있음)

   - java.io.ObjectInputStream
     ObjectInputStream 클래스는 ObjectOutputStream 클래스에 의해 (바이트기반)
     파일에 저장되어 있는 객체나 네트워크를 통해 전달된 객체의
     직렬화를 해제하는 기능을 제공한다.
     단, java.io.Serializable(직렬화가능한) 인터페이스와
     java.io.Externalizable(영속성이 있는) 인터페이스를 지원해주는 객체에 대해서만
     사용이 가능하다.
...
public class Test171
{
   ...
   {
      // 메인메소드가 어떤 환경에서 실행되고있느냐
      String appDir = System.getProperty("user.dir"); // 속성값을 얻어오겠다 사용자의 디렉토리를 
      ...
      //    결과적으로... 『C:\JavaStudy\data\test.ser』 // "\\" 는 \ 하나
      //                    ================== getParent
      // 『ser』 : 직렬화 파일의 확장자
      ...
      //--==>> false 
      // C:\JavaStudy\data\ 경로가 마련되있지 않음을 의미한다. 

          // false가 맞다면
      if (!f0.getParentFile().exists()) //-- 디렉터리 구성이 존재하지않는 상황이 맞다면....
      {
         // 디렉터리 경로를 구성하겠다. (만들겠다.)
      ...
      //-- 파일 전용 출력 스트림(물줄기)객체에 (수도꼭지를 하나 열어서)
      //   f0라는 파일 객체를 띄우겠다...

      // 객체 전용 출력 스트림 생성
      ObjectOutputStream oos = new ObjectOutputStream(fos);
      //-- 객체 전용 출력 스트림(물줄기) 객체로
      //   fos 라는 파일 전용 출력스트림을 감싸겠다...
      
      // 위의 line 147 + line 152 과 동일한 구문 
      ...
      oos.writeObject(h1);       //-- 생성된 스트림 객체(oos)에 내보낼 객체를 기록
      ...
      //--------------------------------------------------------------- 직렬화
      // h1에 put한 변수들이 직렬화되서 test.ser로 내보내짐!!
      ...
      if (f0.exists()) //- f0 파일 객체가 존재한다면...
      {
         ...
         // 읽어들인 객체(Object)를 다운캐스팅(Hashtable 이라는 원래의 타입)하여
         ...
         {
            //key = (String)e.nextElement(); // o -> s 다운캐스팅
