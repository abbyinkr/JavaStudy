...
class CircleTest2
{
	...
	void input()
	{
		...
		r = sc.nextInt();
		//this.r 해도 됨 (매개변수가 따로 없어서 오류안남. 안써도 되서 생략한거임)
	}
	*/
	...
	void setR(int r)
	{
		// r = r;
		//--이와 같은 형태로 표기하게 되면
		//	멤버 변수와 지역변수가 같은 이름을 갖고 있는 상황이기 때문에
		//	둘 다 같은 지역 변수로 인식하여 처리한다.

		//CircleTest2.r = r; (클래스 이름 직접 표기x)
		this.r = r;
		// 『this』는 자기 자신을 나타내는 예약어.
		//-- 멤버 변수를 지역 변수와 구분하기 위하여 클래스의 멤버 변수 앞에 『this』 표기
		//-- 지역변수가 있기때문에 this. 생략 불가!
	}
