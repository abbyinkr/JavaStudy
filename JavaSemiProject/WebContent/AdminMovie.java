  ...
  public static void registerMovie() 
  {
	 ...
	 do
	{
		...
		while (true)		// 예외처리를 하기 위한 반복문 
		{
			 ...
			 {
				// 입력한 개봉일자 월만 가져오기 
				 mm = openDate.substring(0,2);		// 사용자가 입력한 일자 월만 잘라서 mm 변수에 저장
				 dd = openDate.substring(3,5);		// 사용자가 입력한 일자 일만 잘라서 dd 변수에 저장

				 String hi = openDate.substring(2,3);
		  ...
		  // 입력 받은 개봉일 유효성 검사
		 if (omm<1 || omm>12)				// 월이 1~12가 아니라면 
			 flag = true;
		 if (odd>31)						// 일이 31일이 넘으면
			 flag = true;
		 else if (omm==2 && odd>28)			// 2월일시 28일이 넘으면  
			 flag = true;
		 else if ((omm==4 || omm==6 || omm==9 || omm==11) && odd>30)	// 4, 6, 9. 11월일시 30일이 넘으면 
			 flag = true;
	 ...
	 do
	 {
		 ...
		 while (true)		// 예외처리를 하기 위한 반복문 
		 {
			 ...
			 try
			 {
				 ...
				 // 입력한 개봉일자 월만 가져오기 
				 mm = endDate.substring(0,2);		// 사용자가 입력한 일자 월만 잘라서 mm 변수에 저장
				 dd = endDate.substring(3,5);		// 사용자가 입력한 일자 월만 잘라서 mm 변수에 저장
